	...
	public static function build( string $ability, $input ): array {
		...
		if ( null === $target || ! is_array( $target['state'] ) ) {
			return array(
				...
				'fields'  => $fields,
				'diff'    => array(),
				'notice'  => __( 'Arguments only — the effect of this ability could not be verified.', 'mcp-ability-guard' ),
			);
		}
		...
		return array(
			...
			'notice' => $diff
				? ''
				: __( 'The supplied values match the current state — this change would alter nothing.', 'mcp-ability-guard' ),
		);
	}
