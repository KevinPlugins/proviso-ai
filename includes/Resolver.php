...
final class Resolver {
	...
	public static function check_precondition( ?array $stored, $input ) {
		...
		if ( null === $now ) {
			return new \WP_Error(
				'proviso_target_gone',
				__( 'The object this change targets no longer exists.', 'proviso-ai' )
			);
		}
		...
		if ( $now['hash'] !== $stored['hash'] ) {
			return new \WP_Error(
				'proviso_stale',
				__( 'Someone has changed this since the request was queued, so the preview above no longer matches reality. Applying it would discard whatever was done in the meantime. Discard this request and have the agent start again from the current state.', 'proviso-ai' )
			);
		}
